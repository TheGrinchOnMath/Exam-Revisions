...
def create_text_function_window(function_name:str):
    ...
    titre_fonction = Label(fen2,width=1000,text="code de " +function_name,font='Georgia 15',bg='#328563',fg='white')
    titre_fonction.pack()
    scrollbar = Scrollbar(fen2,	bg='#328563')
    scrollbar.pack(side = RIGHT, fill = Y)
    le_texte = Listbox(fen2, yscrollcommand= scrollbar.set,width=1000-16,bg='#2E2E2E',fg='white',font='Arial 12')
...
def uwu(i):
    ...
    for x in range(len(LA_GRILLE)-1):
        ...
        if not posx == False and not posy == False:
            canvas.create_rectangle(posx[0],posy[0],posx[1],posy[1],fill='#82AEA5',outline='#82AEA5')
            break
fen.bind('<Motion>',uwu)



Barre_du_haut = Label(fen,text="Quelle méthode voulez-vous utiliser ?",font='Georgia 18',fg='white',bg="#528A80",height=3,width=int(largeur/9)).place(in_=canvas,x = -180)
button_1 = Button(fen,text="Bracketing",font='Georgia 12',bg='#B1EAE2',height=4,width=16,relief=GROOVE,command=button_1)
button_1.place(in_=canvas,x=largeur*3/16, y=hauteur*3/16+30)
button_2 = Button(fen,text="Dichotomie",font='Georgia 12',bg='#B1EAE2',height=4,width=16,relief=GROOVE,command=button_2)
button_2.place(in_=canvas,x=largeur*10/16, y=hauteur*3/16+30)
button_3 = Button(fen,text="Regula falsi",font='Georgia 12',bg='#B1EAE2',height=4,width=16,relief=GROOVE,command=button_3)
button_3.place(in_=canvas,x=largeur*3/16, y=hauteur*7/16+30)
button_4 = Button(fen,text="Newton-Raphson",font='Georgia 12',bg='#B1EAE2',height=4,width=16,relief=GROOVE,command=button_4)
button_4.place(in_=canvas,x=largeur*10/16, y=hauteur*7/16+30)
button_5 = Button(fen,text="Sécante",font='Georgia 12',bg='#B1EAE2',height=4,width=16,relief=GROOVE,command=button_5)
button_5.place(in_=canvas,x=largeur*3/16, y=hauteur*11/16+30)
button_6 = Button(fen,text="Traceur de courbe",font='Georgia 12',bg='#B1EAE2',height=4,width=16,relief=GROOVE,command=button_6)
